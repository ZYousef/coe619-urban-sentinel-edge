...
class AccidentDetectionSystem:
    ...
    def __init__(self, debug=False):
        ...
        self.threads = {}
        self._accident_unresolved = threading.Event()  # New flag to track unresolved accidents
    ...
    def send_accident_event(self, buffer, frame=None):
        """
        Sends an accident event to the remote API and polls for resolution every 30 minutes.
        Blocks frame processing until resolved.
        """
        try:
            ...
            event_data = {
                ...
                "node_id": self.config.get("Node", "ID"),
                "event_timestamp": str(int(time.time())),
            ...
            if self.debug:
                ...
                self.state.update_accident_state(event_status="reported")
                self._accident_unresolved.set()  # Simulate unresolved state in debug
            else:
                result = self.api_client.send_accident_event(event_data)
                if result["success"] and result["event_id"] is not None:
                    self.state.update_accident_state(event_status="reported")
                    self._accident_unresolved.set()  # Mark accident as unresolved
                    logger.info("Accident reported and state updated. Checking resolution...")

                    # Poll for resolution every 30 minutes
                    check_interval = 1800  # 30 minutes in seconds
                    while not self.shutdown_event.is_set():
                        if self.api_client.check_accident_resolved(result["event_id"]):
                            logger.info("Accident event validated. Entering cooldown.")
                            self.state.update_accident_state(event_status="validated")
                            self._accident_unresolved.clear()  # Accident resolved, resume processing
                            break
                        else:
                            logger.info("Accident not yet validated. Waiting 30 minutes before next check.")
                            if self.shutdown_event.wait(check_interval):
                                logger.info("Shutdown requested during resolution check.")
                                self._accident_unresolved.clear()  # Clear on shutdown
                                break
                else:
                    logger.error(f"Failed to send accident event to API: {result}")
        except Exception as e:
            logger.error(f"Error preparing accident report: {e}")
            self._accident_unresolved.clear()  # Ensure processing resumes on error
    ...
    def process_frames(self):
        ...
        detected in multiple consecutive frames, sends an event. Skips
        processing if an accident is unresolved.
        """
        ...
        while not self.shutdown_event.is_set():
            if self._accident_unresolved.is_set():
                logger.debug("Accident unresolved, skipping frame processing.")
                if self.shutdown_event.wait(5 if not self.debug else 1):
                    break
                continue

            if self.state.is_in_cooldown(self.accident_cooldown):
                last_accident_time = self.state.last_accident_time or 0
                remaining = int(self.accident_cooldown - (time.time() - last_accident_time))
